...
/* Gendered/honorific variants OpenCC deliberately leaves alone (they
   are not trad/simp pairs) but matching must fold: 就是愛妳 vs
   NetEase's 就是爱你. */
const VARIANTS: Record<string, string> = { 妳: '你', 祢: '你', 牠: '它' };
...
/* Loose comparison for cross-platform title/artist matching: lowercase,
   ...
   Also used by the lyrics route's LRCLIB search fallback. */
export function looseNorm(s: string): string {
  return t2s(s.toLowerCase())
    .replace(/[\s　·•・,，.。'’"“”!！?？\-—–()（）\[\]【】]/g, '')
    .replace(/./gu, (ch) => VARIANTS[ch] ?? ch);
}
...
export async function searchNeteaseSongId(
  ...
  for (const song of songs) {
    ...
    if (artistOk) return String(song.id);

    /* Runtime is language-independent — rescues artist-name mismatches
       (何韻詩 vs Denise Ho) that no charset folding can connect. */
    const durationOk =
      durationMs != null &&
      typeof song.duration === 'number' &&
      song.duration > 0 &&
      Math.abs(song.duration - durationMs) <= DURATION_TOLERANCE_MS;
    if (durationOk) return String(song.id);
  }
  return null;
}
